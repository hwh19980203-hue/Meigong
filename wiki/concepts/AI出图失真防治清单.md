...
当首图已经合格，但 A+、场景图或广告图继续使用整图生成/改图时，模型会重新绘制产品。即使字母、数量和大体颜色正确，也可能改变轮廓、比例、边缘、孔位、图案和连接方式。多轮 edit 会继续累积这种漂移。

> [!danger] 禁止做法
> 不得把合格首图仅作为参考图，然后要求模型重新生成带产品的完整 A+ 场景。参考图不是产品锁定机制。

> [!success] 正确做法
> 合格产品必须先抠图并锁定产品像素；AI 只生成背景。最终通过分层合成把固定产品放入场景。
...
### 案例 3：773 God Bless 拉旗 — A+ 衍生图产品变形

**问题：** 首图产品外形可用，但 A+ 整图生成后，旗片比例、波浪边、尖角、孔位、十字架和字母造型发生变化。

**根因：** 生成 A+ 时让 AI 同时重绘产品与场景；后续又多轮修改数量和十字架位置，导致产品几何结构累计漂移。质检只检查了数量和文字，没有执行产品母版对照。
...
3. AI 只生成无产品背景
4. 使用分层合成制作 A+
5. 增加轮廓、比例、边缘、孔位和局部造型硬门禁

**沉淀位置：** [[wiki/analyses/2026-06-22-773-A+产品变形失误复盘]]
